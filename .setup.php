...
class set {
  public function Setup() {
    ...
    // adding bin file
    if ($system=="termux") {
      system("mv -v modules/Ip-ad-Tracker /data/data/com.termux/files/usr/bin/");
      system("mv -v modules/trace /data/data/com.termux/files/usr/bin/");
      system("chmod +x /data/data/com.termux/files/usr/bin/Ip-ad-Tracker trace");
      system("chmod +x /data/data/com.termux/files/usr/bin/trace");
    } elseif ($system=="ubuntu") {
      system("sudo mv -v modules/Ip-ad-Tracker /usr/bin/");
      system("sudo mv -v modules/trace /usr/bin/");
      system("sudo chmod +x /usr/bin/Ip-ad-Tracker");
      system("sudo chmod +x /usr/bin/trace");
    } else {
      system("mv -v modules/Ip-ad-Tracker /usr/bin/");
      system("mv -v modules/trace /usr/bin/");
      system("chmod +x /usr/bin/Ip-ad-Tracker");
      system("chmod +x /usr/bin/trace");
    }

    // copy files from Ip-ad-Tracker to .Ip-ad-Tracker directory.
    if ($system=="termux") {
      system("mkdir /data/data/com.termux/files/usr/share/Ip-ad-Tracker");
      system("chmod +x * *.* .*.*");
      system("mv -v * *.* .*.* /data/data/com.termux/files/usr/share/Ip-ad-Tracker/");
    } elseif ($system=="ubuntu") {
      system("sudo mkdir /usr/share/Ip-ad-Tracker/");
      system("sudo chmod +x * *.* .*.*");
      system("sudo mv -v * *.* .*.* /usr/share/Ip-ad-Tracker/");
    } else {
      system("mkdir /usr/share/Ip-ad-Tracker");
      system("chmod +x * *.* .*.*");
      system("mv -v * *.* .*.* /usr/share/Ip-ad-Tracker/");
    }
    
    // removing Ip-ad-Tracker directory
    if ($system=="termux") {
      system("cd .. && rm -rf Ip-ad-Tracker");
    } elseif ($system=="ubuntu") {
      system("cd .. && sudo rm -rf Ip-ad-Tracker");
    } else {
      system("cd .. && rm -rf Ip-ad-Tracker");
    }
  }
...
EOL;
    ...
    if (file_exists("/usr/bin/Ip-ad-Tracker") || file_exists("/data/data/com.termux/files/usr/bin/Ip-ad-Tracker")) {
      echo "\033[01;32m      Ip-ad-Tracker installed Successfully !!!\033[00m\n";
      echo <<<EOL
...
EOL;
      ...
      echo "\n\n\033[01;31m  Sorry Ip-ad-Tracker is not installed !!!\033[00m";
    }
  }
}
$a=new set;
